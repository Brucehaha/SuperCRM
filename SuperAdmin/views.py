...
def table_order_by(request, queryset, admin_class):
    ...
    if order_id:
        if order_id.startswith('-'):
            ...
            admin_class.order_by['_o'] = order_id.lstrip('-')
            return "-%s" % condition
        else:
            condition = admin_class.list_display[int(order_id)]
...
def table_list(request, app_name, model_name):
    ...
    if queryset.last() is not None:
        ...
        order_condition = table_order_by(request, queryset, admin_class)
        if hasattr(admin_class, 'order_condition'):
            if order_condition == '':
                order_condition = admin_class.order_condition
        admin_class.order_condition = order_condition
        if order_condition != '':
            queryset = queryset.order_by(order_condition)
